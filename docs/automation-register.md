...
| Execution | GitHub Actions runner, `scripts/publish.js` then `scripts/newsletter.js` |
| Permissions | Repo contents write (built in GITHUB_TOKEN); secret `BUTTONDOWN_API_KEY` |
| Inputs | Article files with `status: review` or `scheduled`, `date` on or before today, gate rules; `ops/state.json` |
| Outputs | One article flipped to `published`, commit, Netlify deploy, one Buttondown email to all subscribers, state updated |
...
| Permissions | Secrets `GOATCOUNTER_API_TOKEN` (read only) and `BUTTONDOWN_API_KEY`; repo write for the snapshot commit |
...
| Buttondown API key | GitHub Actions secret | Actions runner only |
| GoatCounter API token | GitHub Actions secret | Actions runner only |
